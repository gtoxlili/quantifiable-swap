...
type gainLoss struct {
	averageGain, averageLoss float64
}
...
type RSI[T Number] struct {
	...
	// Stores the last 5 RSI values for data analysis (queue)
	recentRSIQueue []float64
}
...
func NewRSI[T Number](period int, seq sequence.Sequence[T]) (Indicator[T], error) {
	...
	rsi := &RSI[T]{
		period:             period,
		Sequence:           seq,
		historicalAverages: make([]gainLoss, scale),
		recentRSIQueue:     []float64{-1, -1, -1, -1, -1},
	}
...
func (rsi *RSI[T]) Update(ctx context.Context) (*sequence.Candle[T], error) {
	...
	prevAvg := rsi.historicalAverages[index]

	// 使用 RSI 标准的平滑公式更新当前时间序列的平均涨幅和平均跌幅：
	// newAvg = (prevAvg*(period-1) + currentValue) / period
	newAvgGain := (prevAvg.averageGain*float64(rsi.period-1) + currentGain) / float64(rsi.period)
	newAvgLoss := (prevAvg.averageLoss*float64(rsi.period-1) + currentLoss) / float64(rsi.period)

	// 更新该时间序列对应的历史状态
	rsi.historicalAverages[index] = gainLoss{averageGain: newAvgGain, averageLoss: newAvgLoss}
...
func (rsi *RSI[T]) CurrentVal() float64 {
	...
	rs := avg.averageGain / avg.averageLoss
	return 100 - 100/(1+rs)
}
...
func (rsi *RSI[T]) computeHistoricalRSI() error {
	...
	for a := 0; a < scale; a++ {
		...
		rsi.historicalAverages[a] = gainLoss{avgGain, avgLoss}
	}
	return nil
}
